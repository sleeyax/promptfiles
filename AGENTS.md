...
Agents are installed by symlinking into `~/.config/Code/User/prompts/`. Skills (used as Claude Code's replacement for slash commands in this repo) are installed via the [`skills`](https://www.npmjs.com/package/skills) npm CLI; agents are still installed by symlink.
